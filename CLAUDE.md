...
⚠️ **MCM 的 md 转换件缺 32 章**（全书 154 章，`MCM分章/` 只有 122 章）。缺的多是临床核心内容：
第 23 章链球菌属、32 章结核分枝杆菌、38 章嗜血杆菌属、51 章军团菌属、59 章螺杆菌属、
71 章抗菌药物耐药机制、113 章抗病毒药物、132 章抗真菌药物，以及**大部分病毒章与全部蠕虫章**。
完整清单见 `01-临床微生物学手册-第12版/源文件勘误.md` 第五节。

**`grep 关键词 完整版.md` 有命中不等于该章存在**——目录、卷末索引、别章的交叉引用都会命中。
判断某章在不在，看 `MCM分章/` 有没有对应编号的文件。缺的章要从两卷源 PDF 按页码渲染页面图来读
（`pdftoppm -r 200 -f N -l M -png`）。查不到就不写，别拿别章的泛泛描述硬凑。
