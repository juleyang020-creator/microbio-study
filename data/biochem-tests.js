...
window.DB.biochemTests = [
  ...
  },
  {
    ...
    小节: [
      ...
      { 标题: '临床意义', 正文: '赖氨酸/鸟氨酸脱羧酶是肠杆菌目鉴别的关键试验，须按种（复合群）判读，不可按属推断。LDC（+）/ODC（±）：大肠埃希菌（ODC 约 65% 阳性，不宜作固定型别）。LDC（+）/ODC（−）：肺炎克雷伯菌、产酸克雷伯菌——ODC 阴性正是克雷伯菌与肠杆菌属的经典鉴别点。LDC（−）/ODC（+）：阴沟肠杆菌复合群（ADH 亦为 +）。LDC（+）/ODC（+）：多数沙门菌（伤寒沙门菌 ODC 阴性）、黏质沙雷菌、产气克雷伯菌（2017 年由产气肠杆菌改隶克雷伯菌属）。LDC（−）：志贺菌属（全属）、变形杆菌属。单一脱羧酶结果不足以定种，须与吲哚、VP、动力、枸橼酸盐组合判读。' }
    ]
  },
  ...
  },
  {
    ...
    小节: [
      ...
      { 标题: '结果判读', 正文: '试剂层出现樱红色环为阳性（+）；黄色（不变色）为阴性（−）。Ehrlich 试剂与 Kovacs 同用对二甲氨基苯甲醛，仅萃取用醇不同，对弱产吲哚菌（如厌氧菌、嗜血杆菌）更敏感；另有对二甲氨基肉桂醛（DMACA）点滴法，灵敏度更高。' },
  ...
  },
  {
    ...
    小节: [
      ...
      { 标题: '临床意义', 正文: '赖氨酸（+）：大肠埃希菌（约 90%）、肺炎克雷伯菌、产酸克雷伯菌、产气克雷伯菌、黏质沙雷菌、多数沙门菌。赖氨酸（−）：志贺菌属（全属）、枸橼酸杆菌属（多数）、变形杆菌属、阴沟肠杆菌复合群。注意「肠杆菌属」不可整属概括——阴沟肠杆菌为 LDC（−），而原产气肠杆菌（LDC +）已改隶克雷伯菌属。志贺菌属各群 LDC 均阴性，故赖氨酸不能用于属内鉴别；属内鉴别看鸟氨酸脱羧酶。' }
    ]
  },
  {
    ...
    小节: [
      ...
      { 标题: '临床意义', 正文: '鸟氨酸（+）：多数沙门菌（伤寒沙门菌除外）、阴沟肠杆菌复合群、黏质沙雷菌、奇异变形杆菌、摩根摩根菌、宋内志贺菌。鸟氨酸（−）：肺炎克雷伯菌、产酸克雷伯菌、普通变形杆菌、痢疾志贺菌、福氏志贺菌、鲍氏志贺菌。可变（±）：大肠埃希菌（约 65% 阳性）。记忆点：宋内志贺菌是志贺菌属中唯一常规 ODC 阳性者（并 ONPG +、迟缓发酵乳糖），是属内鉴别的标志；奇异变形杆菌（+）与普通变形杆菌（−）亦以此区分。志贺菌定种仍须靠血清学分群或质谱/分子方法。' }
    ]
  },
  {
