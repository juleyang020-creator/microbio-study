...
- **异常药敏速查** (`#/ast-alerts`) — 常见需修正/警示的药敏结果组合（如 MRSA 报敏感 β-内酰胺、肠球菌报敏感头孢等）。13 条规则，按「必须修正 / 需复核 / 限制报告」三等级 + 菌名/药物/机制关键词双维筛选，每条含触发条件、异常结果、处理建议与依据，底部附 CLSI/EUCAST/CDC 参考口径链接。
