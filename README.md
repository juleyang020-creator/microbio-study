...
4. **生化反应** — 酶活性 / 碳水化合物 / 氨基酸 / 生长特性 / 溶血与特殊反应
5. **检测卡** — 药敏卡（革兰阴性 / 阳性 / 真菌 / 链球菌）+ 鉴定卡（VITEK 2 原理与选卡），同一模块内分组
6. **质控菌株** — ATCC 质控菌株及其 MIC / 抑菌圈可接受范围（CLSI M100 / M45 / M27M44S / M38M51S）
7. **试验** — 药敏试验方法（KB 法 / E-test / MIC）、耐药表型检测、鉴别试验
8. **染色** — 常规染色 / 特殊结构染色 / 真菌染色 / 血液与寄生虫染色
9. **培养基** — 基础与营养 / 选择与鉴别 / 特殊培养基

各模块互相打通：在一个条目里声明 `关联`，相关条目会在两边自动互相显示。

## 两个对比工具（顶部「快捷工具」栏）

- **生化鉴定** (`#/compare`) — 勾选 2 个以上细菌，并排比较生化反应结果，差异项自动高亮（鉴别要点）。
...
- **菌名速查** (`#/microbe-names`) — 4555 条微生物名称（中文 + 拉丁）索引，含 Bruker MBT 主库 2969 种。命中本库的 186 条名称（覆盖全部 185 个菌，立克次体有属名/spp. 两条记录）跳应用内详情页（离线，标「本库」）；未收录者按拉丁名完整度分流——全写「属 种」的 3311 条跳 NCBI Taxonomy 精确条目，缩写属名/仅属名的 1058 条跳 PubMed 文献检索（均为权威源、国内可达）。同名异译（如「嘴突脐孢」之于「喙状明脐菌」）收在 `别名` 字段里，一并参与检索——照着 MALDI 报告上的名字也能查到。
...
> 下表数字由 `data/` 实测得出，改数据后请一并更新。

### 主模块数据
| 文件 | 模块 | 条目量 |
|---|---|---|
| `data/microbes.js` | 微生物分类（精选临床常用） | 185 |
| `data/microbe-names.js` | 菌名速查索引（本库详情 / NCBI Taxonomy / PubMed，按拉丁名字母序） | 4555 |
...
| 文件 | 用途 | 键数 |
|---|---|---|
| `data/morphology.js` | 形态学特征（镜下 / 培养） | 174 |
| `data/biochem.js` | 生化反应结果（用于详情页与对比） | 111 |
| `data/differential.js` | 相似菌鉴别要点 | 130 |
| `data/treatment.js` | 治疗要点（按微生物 id 索引） | 181 |
| `data/breakpoints.js` | CLSI M100/M45（细菌）· M27M44S/M38M51S（真菌）药敏折点（按菌组） | 32 组 / 298 药物行 |
| `data/eucast-breakpoints.js` | EUCAST v16.1 MIC 折点，与 CLSI 并排对照 | 28 组 |
...
node --test
```
共 119 个用例，覆盖：核心逻辑（关联、搜索、分类校验）、视图模型（详情/对比/折点/天然耐药/MIC 与抑菌圈判读/异常药敏筛选）、数据完整性（id 唯一、悬空关联、未匹配分类、辅助数据键合法、机制图文件存在、异常药敏规则字段与等级合法、离线预缓存清单、高风险治疗要点与资源缓存回归检查等）。
